...
async function handleUpdate(request: NextRequest, id: string) {
  ...
  const { rows } = await db.query(
    `UPDATE employees SET name=$1, email=$2, department=$3, phone=$4, joining_date=COALESCE($5, joining_date)
     WHERE id=$6 AND tenant_id IS NULL RETURNING id, name, email, department, phone, joining_date, is_active, created_at`,
    [name.trim(), email?.trim() || null, department.trim(), phone?.trim() || null, joining_date || null, id]
  )
