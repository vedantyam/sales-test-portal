...
export default function EmployeesPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { name: string; department: string } }) => {
      await api.patch(`/admin/employees/${id}`, data)
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['employees'] })
  ...
  const regenKeyMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await api.post(`/admin/employees/${id}/regenerate-key`)
      return res.data
    },
    onSuccess: (data, id) => {
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((emp) => (
                  <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-gray-500">{formatDate(emp.created_at)}</td>
                    <td className="px-5 py-3">
